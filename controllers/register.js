module.exports.handleRegister = (req, res, db, bcrypt) => {
    ...
    if (!name || !email || !password) {
        return res.status(400).json('Incorrect form submission - missing user information.');
    }

    const formatPass = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");
    const formatEmail = new RegExp("^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$");
    //Need to add password match later: !password.match(formatPass) 
    if(!email.match(formatEmail)) {
        const message = 'Error with credentials - email must be formatted as example@example.com - password..';
